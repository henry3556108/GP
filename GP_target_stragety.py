...
def append_trees_to_csv(trees: List[Node], filename='output.csv', generation:int=0):
    ...
    try:
        ...
        df_existing = pd.DataFrame(columns=['generation', 'file_name', 'mo_fitenss_score'])
    ...
    for tree in trees:
        ...
        data['mo_fitenss_score'].append(tree.fitness_score[0] if len(tree.fitness_score) > 0 else None)
...
if __name__ == "__main__":
    ...
    # print(len(get_population_from_folder()))   
    ...
    for generation in range(1, generation_times + 1):
        ...
        print(f"population size from pending:{len(population)}")
        # break
        append_trees_to_csv(population, record_csv_path, generation)
        scores = [indivudial.fitness_score[0] for indivudial in population]
        print(f"max score is: {max(scores)}")
        population = filter_trees(population, 0.9)
        scores = [indivudial.fitness_score[0] for indivudial in population]
        print(f"max score is: {max(scores)}")
        print(f"before survivor population size:{len(population)}")
        population = survivor_selection(population, offsprint_amount, population_size * (1 - reborn_rate))
        scores = [indivudial.fitness_score[0] for indivudial in population]
        ...
        print(f"after survivor population size:{len(population)}, offspring size{len(offsprints)}")
        reborn = []
        if len(offsprints) + len(population) < population_size:
            ...
            offsprints += reborn
        print(f"reborn amount {len(reborn)}")
